...
/// 图片转矢量svg
///
/// https://github.com/GreenAppers/flutter_potrace
...
Path potracePath(
  ...
  List<_Path> pathlist = _BitmapToPathlist(bm, turnpolicy, turdsize).run();
  _ProcessPath(optcurve, alphamax, opttolerance).run(pathlist);
  return _GetPath(bm, 1.0 /*精度大小*/, "", pathlist).run();
}
...
class _DPoint {
  ...
  _DPoint.fromPoint(_Point p) : this(p.x.toDouble(), p.y.toDouble());
...
class _Bitmap {
  ...
  _Bitmap.fromImg(img.Image image, {int grayThreshold = 128 /*小于这个值, 视为黑色*/})
    ...
    for (int i = 0; i < image.length; i++) {
      ...
      data[i] = (color < grayThreshold ? 1 : 0);
    }
  }

  bool at(int x, int y) =>
      (x >= 0 && x < w && y >= 0 && y < h) && data[w * y + x] == 1;
...
class _Path {
  ...
  int area = 0, minX = 100000, minY = 100000, maxX = -1, maxY = -1;
  late String sign;
}
...
class _Quad {
  ...
  double at(int x, int y) => data[x * 3 + y];
}

class _Opti {
  num pen = 0, t = 0, s = 0, alpha = 0;
  List<_DPoint> c = List<_DPoint>.filled(2, _DPoint());
}
...
class _ProcessPath {
  ...
  static void calcLon(_Path path) {
    ...
    List<int> pivk = List.filled(n, 0),
        nc = List.filled(n, 0),
        ct = List.filled(4, 0);
    path.lon = List.filled(n, 0);
  ...
  static void bestPolygon(_Path path) {
    ...
    List<num> pen = List.filled(n + 1, 0);
    List<int> prev = List.filled(n + 1, 0),
        clip0 = List.filled(n, 0),
        clip1 = List.filled(n + 1, 0),
        seg0 = List.filled(n + 1, 0),
        seg1 = List.filled(n + 1, 0);
    num thispen, best;
  ...
  static void adjustVertices(_Path path) {
    ...
    List<int> po = path.po;
    List<_Point> pt = path.pt;
    List<_DPoint> ctr = List.filled(m, _DPoint()),
        dir = List.filled(m, _DPoint());
    List<_Quad> q = List.filled(m, _Quad());
    List<double> v = List.filled(3, 0);
    int i, j, k, l;
    _DPoint s = _DPoint();
    num d;
    ...
    _Quad Q;
    _DPoint w;
    double dx, dy, det, min, cand, xmin, ymin;
    int z;
    for (i = 0; i < m; i++) {
      Q = _Quad();
      w = _DPoint();
  ...
  void optiCurve(_Path path) {
    ...
    List<int> pt = List.filled(m + 1, 0),
        len = List.filled(m + 1, 0),
        convc = List.filled(m, 0);
    List<num> pen = List.filled(m + 1, 0), areac = List.filled(m + 1, 0);
    List<_Opti> opt = List.filled(m + 1, _Opti());
    _Opti o = _Opti();
    _DPoint p0;
    num area, alpha;
    _Curve ocurve;
    ...
    for (j = 1; j <= m; j++) {
      ...
      for (i = j - 2; i >= 0; i--) {
        bool r = optiPenalty(path, i, mod(j, m), o, opttolerance, convc, areac);
        if (r) {
          break;
        }
        if (len[j] > len[i] + 1 ||
    ...
    om = len[m];
    ocurve = _Curve(om);
    List<num> s = List.filled(om, 0), t = List.filled(om, 0);
...
class _GetSVG {
  ...
  String bezier(_Curve curve, int i) {
    return 'C ${(curve.c[i * 3 + 0].x * size).toStringAsFixed(3)} ${(curve.c[i * 3 + 0].y * size).toStringAsFixed(3)},${(curve.c[i * 3 + 1].x * size).toStringAsFixed(3)} ${(curve.c[i * 3 + 1].y * size).toStringAsFixed(3)},${(curve.c[i * 3 + 2].x * size).toStringAsFixed(3)} ${(curve.c[i * 3 + 2].y * size).toStringAsFixed(3)} ';
  }

  String segment(_Curve curve, int i) {
    return 'L ${(curve.c[i * 3 + 1].x * size).toStringAsFixed(3)} ${(curve.c[i * 3 + 1].y * size).toStringAsFixed(3)} ${(curve.c[i * 3 + 2].x * size).toStringAsFixed(3)} ${(curve.c[i * 3 + 2].y * size).toStringAsFixed(3)} ';
  }

  String path(_Curve curve) {
    int n = curve.n;
    String p =
        'M${(curve.c[(n - 1) * 3 + 2].x * size).toStringAsFixed(3)} ${(curve.c[(n - 1) * 3 + 2].y * size).toStringAsFixed(3)} ';
  ...
  String run() {
    ...
    String svg =
        '<svg id="svg" version="1.1" width="$w" height="$h" xmlns="http://www.w3.org/2000/svg">';
  ...
  void path(_Curve curve) {
    ...
    for (int i = 0; i < n; i++) {
      switch (curve.tag[i]) {
        ...
        case 'CORNER':
          ret.lineTo(curve.c[i * 3 + 1].x * size, curve.c[i * 3 + 1].y * size);
          ret.lineTo(curve.c[i * 3 + 2].x * size, curve.c[i * 3 + 2].y * size);
          break;
      }
    }
  }
