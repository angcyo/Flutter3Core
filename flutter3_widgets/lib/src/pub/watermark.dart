...
class TextWaterMarkPainter extends WaterMarkPainter {
  ...
  Size paintUnit(Canvas canvas, double devicePixelRatio) {
    ...
    //通过三角函数计算旋转后的位置和size
    final orgSin = sin(radians);
    final sinAbs = orgSin.abs();
    final cosAbs = cos(radians).abs();

    final width = textWidth * cosAbs;
    final height = textWidth * sinAbs;
    final adjustWidth = textHeight * sinAbs;
    final adjustHeight = textHeight * cosAbs;
...
class StaggerTextWaterMarkPainter extends WaterMarkPainter {
  ...
  Size paintUnit(Canvas canvas, double devicePixelRatio) {
    ...
    // 返回两个文本水印所占用的总大小
    return Size(
      vertical ? max(size1.width, size2.width) : size1.width + size2.width,
      vertical ? size1.height + size2.height : max(size1.height, size2.height),
    );
  }
