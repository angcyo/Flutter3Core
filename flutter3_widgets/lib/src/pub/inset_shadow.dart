...
/// - [InsetBoxDecoration]
///   - [InsetBoxShadow]
class InsetBoxDecoration extends BoxDecoration {
...
Rect _unionRects(Rect a, Rect b) {
  ...
  final left = min(a.left, b.left);
  final top = min(a.top, b.top);
  final right = max(a.right, b.right);
  final bottom = max(a.bottom, b.bottom);

  return Rect.fromLTRB(left, top, right, bottom);
}

class InsetBoxShadow extends BoxShadow {
  ...
  static List<InsetBoxShadow>? lerpList(
    ...
    a ??= <InsetBoxShadow>[];
    b ??= <InsetBoxShadow>[];
    final int commonLength = min(a.length, b.length);
    return <InsetBoxShadow>[
  ...
  @override
  String toString() =>
      'InsetBoxShadow($color, $offset, ${debugFormatDouble(blurRadius)}, ${debugFormatDouble(spreadRadius)}), $blurStyle, $inset)';
}
