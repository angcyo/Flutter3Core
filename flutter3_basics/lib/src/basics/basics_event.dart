...
bool get isCtrlPressed => isKeyPressed(
  key: LogicalKeyboardKey.control,
  keys: [LogicalKeyboardKey.controlLeft, LogicalKeyboardKey.controlRight],
);
...
bool get isAltPressed => isKeyPressed(
  key: LogicalKeyboardKey.alt,
  keys: [LogicalKeyboardKey.altLeft, LogicalKeyboardKey.altRight],
);
...
bool get isMetaPressed => isKeyPressed(
  key: LogicalKeyboardKey.meta,
  keys: [LogicalKeyboardKey.metaLeft, LogicalKeyboardKey.metaRight],
);

/// [HardwareKeyboard.isShiftPressed]
bool get isShiftPressed => isKeyPressed(
  key: LogicalKeyboardKey.shift,
  keys: [LogicalKeyboardKey.shiftLeft, LogicalKeyboardKey.shiftRight],
);
...
bool isSameLogicalKey(LogicalKeyboardKey? key1, LogicalKeyboardKey? key2) {
  ...
  if (key1.isControlKey && key2.isControlKey) {
    return true;
  }
  if (key1.isAltKey && key2.isAltKey) {
    return true;
  }
  if (key1.isMetaKey && key2.isMetaKey) {
    return true;
  }
  return false;
}
...
extension EventIntEx on int {
  ...
  /// 是否是鼠标右键
  bool get isMouseRight =>
      (this & kSecondaryMouseButton) == kSecondaryMouseButton;

  /// 是否是鼠标中键,
  bool get isMouseMiddle => (this & kMiddleMouseButton) == kMiddleMouseButton;
}
...
extension PointerEventEx on PointerEvent {
  ...
  /// 鼠标按下的按钮记录, 因为鼠标弹起时, 按钮记录会重置为0, 所以需要记录一下
  static var _mouseDownButtons = 0;

  /// 保存鼠标按下的按钮记录
  /// [_mouseDownButtons]
  bool get _saveMouseDownButtons {
    if (isMouseEventKind && (isPointerDown || isPointerMove)) {
...
mixin PointerDispatchMixin {
  ...
  /// 是否要忽略当前客户端的手势处理
  /// @return true 忽略客户端
  bool ignoreClientHandlePointer(
    IHandlePointerEventMixin client,
    PointerEvent event,
  ) {
    return !client.enableEventHandled;
  }
}
...
mixin TouchDetectorMixin {
  ...
  /// 长按循环触发器
  @output
  Timer? _loopLongPressTimer;

  //--

  /// 双击事件探测触发的点击次数. 这里可以指定N次触发
  /// - <2: 表示关闭双击检查
  /// - 2: 表示短时间点击2次即触发事件
  @configProperty
  int touchDoubleClickCount = 2;

  /// 是否要检查双击事件
  /// - 关闭双击检查, 点击事件会触发的更快
  bool get checkDoubleClick => touchDoubleClickCount > 1;

  /// 第二次点击抬手时的时间间隔小于此值, 视为双击
  /// - 距离上一次的点击事件不能超过此时, 超过则会重置有效的点击计数
  ///
  /// - [kDoubleTapTimeout]
  /// - [kDoubleTapMinTime]
  @configProperty
  Duration touchDoubleTimeout = Duration(milliseconds: 160);

  /// 上一次有效的手指抬手的时间, null表示还未抬手过
  /// - 双击时, 必须只有一个手势触发. 不支持多个手势下的双击
  /// -
  @output
  int? _lastPointerUpTime;

  /// 已经点击的有效点击次数
  @output
  int _touchDetectorClickCount = 0;

  /// 双击模式下, 点击事件的触发器
  @output
  Timer? _clickDetectorTimer;
  ...
  void _checkClick(PointerEvent event) {
    ...
    if (checkDoubleClick) {
      //双击检查
      _clickDetectorTimer?.cancel();
      //debugger(when: _touchDetectorClickCount > 0);
      final time = nowTime();
      if (_lastPointerUpTime == null ||
          time - _lastPointerUpTime! <= touchDoubleTimeout.inMilliseconds) {
        //第一次点击, 或者有效的点击
        _lastPointerUpTime = time;
        _touchDetectorClickCount++;
      } else {
        //间隔很长, 无效的点击
        _clearDoubleClickCount();
      }
  ...
  void _checkLongPress(PointerEvent event) {
    ...
    if (enableLoopLongPress) {
      _loopLongPressTimer?.cancel();
      _loopLongPressTimer = null;
      _loopLongPressTimer = Timer.periodic(loopLongPressDelay.milliseconds, (
        timer,
      ) {
        onTouchDetectorPointerEvent(event, TouchDetectorType.longPress);
      });
  ...
  /// 清理双击点击计数, 这样会重置双击检查
  void _clearDoubleClickCount() {
    _lastPointerUpTime = null;
    _touchDetectorClickCount = 0;
    _clickDetectorTimer?.cancel();
    _clickDetectorTimer = null;
  }
}
...
mixin MultiPointerDetectorMixin {
  ...
  @dp
  static List<Offset> getPointerDeltaList(
    Map<int, PointerEvent> pointerMoveMap,
  ...
  Offset moveLastDelta() {
    final offsetList = getPointerDeltaList(
      pointerMoveMap,
      pointerMoveLastMap,
      pointerDownMap,
    );
...
mixin TranslateDetectorMixin {
  ...
  /// 当前移动的位置
  Offset _movePosition = Offset.zero;

  /// 入口函数
  @entryPoint
  bool addTranslateDetectorPointerEvent(PointerEvent event) {
    bool handle = false;
    final pointer = event.pointer;
    if (event.isPointerDown) {
      isTranslating = false;
      ...
      if (mdx.abs() >= slopX || mdy.abs() >= slopY) {
        ...
        isTranslating = true;
        _movePosition = localPosition;
        handle = handleTranslateDetectorPointerEvent(event, ddx, ddy, mdx, mdy);
      }
    } else if (event.isPointerFinish) {
      if (isTranslating) {
        ...
        final ddx = localPosition.dx - _downPosition.dx;
        final ddy = localPosition.dy - _downPosition.dy;
        handle = handleTranslateDetectorPointerEvent(event, ddx, ddy, 0, 0);
      }
      _downPointer = -1;
      isTranslating = false;
    }
